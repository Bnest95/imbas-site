...
7. **Every declaration row** the paired analysis owns — Reader Run Declarations `tblMo8qwOVbDacLLd`. This is an **append-only log in a separate table**, one immutable row per declaration event, joined to the pair by value on **(`Open Run ID`, `Targeted Answer Hash`)**. There is no Airtable link field and therefore **no cascade**: deleting the paired-analysis row in item 2 does not touch these rows. One pair owns **as many rows as the person made declarations** — a submission, a later correction, a correction of the correction — so all of them must be found, not the newest one. What they hold is the requester's own words about how they ran the pair: `Declared Model Version` is free text they typed, and `Declared Same Model` / `Declared Edits` / `Stage` / `Actor` are what they reported. It is content, not telemetry, and it goes.

**The one cascade this base does not have.** Items 1–3 each carry their content on the row, so deleting the row is sufficient. Item 7 does not. If a walk deletes the run row and the paired rows and stops, the declarations remain — orphaned, still readable, still the requester's words, and now unreachable through the join that would have found them. Capture the pair's `Open Run ID` and `Targeted Answer Hash` **before** deleting the paired row, or the key to the surviving rows is gone with it.
...
| 3 | Share row(s) | Inspection Shares `tbliYeeM5n0TSVrxf` | **P4 shape:** Share ID; Question (raw); Mode; Receipt Hash (foreign key to the run or paired row — see below); Findings JSON (short verbatim excerpts, **no full answer**); Captured At, AI Model (dated-receipt facts copied from the proof row — §2a); Declaration IDs (identities only, newline-joined, written once at mint — the declarations themselves are row 7, and a share never held their content); Created At (share-mint time); Reviewed Status; Report Flag; Visibility. **Legacy pre-P4 shape (still present):** Share ID; Question, Answer (full raw copy); The Read, What Was Left Out, How It Was Shaped, Inspection Note, Completeness (derived copy); labels; visibility |
...
| 7 | Declaration log | Reader Run Declarations `tblMo8qwOVbDacLLd` | One row per declaration EVENT, append-only. Open Run ID + Targeted Answer Hash (the two-part value join to the pair); Declaration ID, Declaration Version, Declaration Source; Stage, Actor; Declaration Status; **Declared Same Model, Declared Model Version (free text the requester typed), Declared Edits**; Declared At Client, Received At Server; Supersedes Declaration ID. **Separate table, no link field, no cascade** — these rows survive deletion of the paired row unless walked |
...
### 2a. The two dated-receipt fields on a share row

The share page is a dated capture receipt, so a share row carries the two facts that date it. Both are written **once, at mint**, copied from the proof row the mint path already fetched by Receipt Hash, and **never written again** — a product rerun mints a new share row rather than updating this one, which is what lets the page promise the record does not change. Concurrent mints of one receipt copy identical values off the same proof row, so the write is idempotency-safe.

| Field | Copied from | Meaning | Not to be confused with |
|---|---|---|---|
| `Captured At` (`fldFjhbqwmDN0Ku1J`, dateTime, ISO/UTC) | the proof row's `Created` — Reader Runs (single) or Reader Paired Analyses (paired) | **When the inspected answer was captured.** The date the receipt is dated by. | `Created At` |
| `AI Model` | Reader Runs `Inspected AI Model` | The answering system the person running the inspection **declared**. Imbas records it and does not observe it. Written for single-mode only — Reader Paired Analyses has no model field, so a paired share leaves it empty and the receipt states the system as unobserved. | an observed or verified model identity |

**`Created At` is share-mint time and stays that way.** It is the sole tiebreaker in `pickCanonicalShare`, which resolves a concurrent-write race by earliest `Created At`; aliasing it to the capture time would give two mints of one receipt an identical value and collapse the tiebreak to record-ID alone. Do not touch, alias, or overload it.

Absent values are **omitted, never written empty** — this base's house rule, and an empty string into a dateTime is a write error rather than a recorded absence. The receipt states an unheld value as unobserved in words (`reader-receipt-page.js`), which is a different claim from an empty slot.

Both fields are **additive**. There is no backfill and no migration script: the table held 0 rows when they were added (2026-08-01, verified live), so no published link changed meaning.

Neither field changes the deletion walk — both live *on* the share row and die with it — but both are named here so a residue check that enumerates what a P4 share holds stays complete.
...
- **Declaration log:** query Reader Run Declarations `tblMo8qwOVbDacLLd` **twice**, because one query cannot prove both things. (a) For each pair, `Open Run ID` **and** `Targeted Answer Hash` = that pair's values — expect **0 rows**; this proves the walk emptied what it aimed at. (b) For `Open Run ID` = the run's `Request ID` alone — expect **0 rows**; this catches declarations belonging to a paired row step 4 failed to find, which the two-part query cannot see because it never learned that pair's hash. A row surviving (b) is an orphan the join can no longer reach, so it will not be found next time either.
...
### 2026-08-03 (UTC) — v4 schema proof, declaration log (write · read · append · delete)

- **Namespace:** `DELETE_TEST_DECL_LOG_20260803T010337Z`, carried in `Open Run ID` and as the prefix of both `Declaration ID`s. `Targeted Answer Hash` = `085d3b22…ce4b`, the real sha256 of the namespace string, so the row carries a hash-shaped value that no answer produces.
- **Not a full closure-set walk, and not a proof of the endpoint.** The declaration code is written but **not deployed**, so there was no paid run and no pipeline-produced pair. This exercises the **new store** — that the live table accepts what the code writes, returns it unchanged, orders it, and gives it up on deletion. The end-to-end walk (run → pair → declarations → share) becomes provable when the endpoint ships; the P2 entry above is the template.
- **Created (direct writes, mirroring what `reader-declaration-log.js` appends):**
  - `recaxIV4r2opmkK7r` — `…Z.d1`, stage `submission`, `Declared Same Model` = `yes`, `Supersedes` = `NO_SUPERSESSION`, `Received At Server` = `2026-08-03T01:03:37.000Z`.
  - `rec8q9CMAkScPn6fD` — `…Z.d2`, the correction: stage `review`, `Declared Same Model` = `not_sure`, `Supersedes` = `…Z.d1`, `Received At Server` = `2026-08-03T01:06:00.000Z`.
- **Read back, three ways:**
  - By the ownership join (`Open Run ID` **and** `Targeted Answer Hash`) after the first write — **1 row**, all twelve read-back fields byte-identical to what was sent. No coercion, no defaulting, no dropped absence token: `NOT_CAPTURED` and `NOT_DECLARED` came back as themselves rather than as empty cells.
