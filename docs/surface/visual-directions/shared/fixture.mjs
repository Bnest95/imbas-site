...
/* ── Z2.3 · the line that teaches a first reader what a mark is ───────────────
 *
 * Recast by the ruling of 2026-08-10. It carried the prototype's wording verbatim
 * until then, and the copy inventory had been flagging it as a denial shape for two
 * passes — twice, since it registers both as `ui.z2.orientation` and as this
 * constant. It survived on the argument that absence is the subject matter rather
 * than a misreading, which is true of the proposition and beside the point about the
 * setting: this pass moved the line into COLD GLANCE, where it sits in the record's
 * own voice beside the forwarded strip rather than as an aside to a runner who has
 * just come from the entry surface. A line in record voice takes the standing copy
 * law.
 *
 * So the two contracted negatives go and nothing else does. `isn't in it` becomes
 * `absent from it`, which is a state rather than an expectation — `missing` would
 * have implied the answer owed the reader that material, which is the insertion-caret
 * claim under different words. `what was there and what wasn't` becomes `both`,
 * which refers to the two referents just named and carries the same proposition: the
 * record covers presence and absence alike.
 *
 * Every proposition of the original survives. A mark has a referent. That referent
 * may sit in the answer. It may equally sit outside it. Imbas records either case.
 * What is gone is the shape, not the meaning. */
