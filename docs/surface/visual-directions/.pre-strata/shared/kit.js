...
(function () {
  ...
  function identity(rec, c) {
    /* SNAPSHOT of the pre-strata build. Kept only so the BEFORE numbers can be
     * re-measured under the final metric definition. Not part of the lane build. */
  ...
  function foot(rec, cls) {
    ...
    return [
      ...
      el("div", { class: c.row, zone: "Z7.3" }, [
        ...
        el("p", { class: c.cite, text: rec.citation }),
      ]),
    ];
  }
  ...
  function boot(direction) {
    ...
    root.setAttribute("data-anatomy-sha", F.anatomy_sha);
