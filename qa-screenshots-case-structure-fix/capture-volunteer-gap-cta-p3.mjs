...
const outDir = path.join(__dirname, 'volunteer-gap-cta-p3');
...
async function captureCta(name, width) {
  ...
  await page.waitForSelector('.vg-cta', { timeout: 10000 });
  await page.evaluate(() => {
    document.querySelector('.vg-cta')?.scrollIntoView({ block: 'center' });
  });
  await wait(500);
  const el = await page.$('.vg-cta');
  await el.screenshot({ path: path.join(outDir, name) });

  const report = await page.evaluate(() => {
    const cta = document.querySelector('.vg-cta');
    const links = [...cta.querySelectorAll('a')];
    const primary = cta.querySelector('.vg-cta__primary');
    ...
    return {
      ...
      order: links.map((a) => ({
        ...
        isPrimary: a.classList.contains('vg-cta__primary'),
      ...
      secondaryIsTransparent: secondaryStyle?.backgroundColor === 'rgba(0, 0, 0, 0)',
...
const mobile = await captureCta('volunteer-gap-cta-mobile-375.png', 375);
const desktop = await captureCta('volunteer-gap-cta-desktop-1440.png', 1440);
