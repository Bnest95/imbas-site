...
const LIFECYCLE_ERR =
  /Requesting main frame too early|TargetCloseError|Protocol error|Target closed|Session closed|browser has disconnected|Navigating frame was detached|Connection closed|ConnectionClosedError/i;

const NAV_OPTS = { waitUntil: 'domcontentloaded', timeout: 60000 };

function logDiag(label, value) {
  console.error(`[final-check] ${label}: ${value}`);
}

logDiag('Node', process.version);
try {
  const pkg = await import('puppeteer/package.json', { with: { type: 'json' } });
  logDiag('Puppeteer', pkg.default?.version || 'unknown');
} catch {
  logDiag('Puppeteer', 'unknown');
}
try {
  logDiag('Browser path', puppeteer.executablePath());
} catch {
  logDiag('Browser path', 'unavailable (set PUPPETEER_EXECUTABLE_PATH)');
}
logDiag('Base URL', base);
...
async function launchBrowser() {
  ...
  const launchOpts = {
    headless: true,
    executablePath,
    // --single-process stabilizes Chrome for Testing in constrained/sandbox hosts
  ...
  console.error('Chrome/Chromium unavailable. Install browser or set PUPPETEER_EXECUTABLE_PATH.');
  console.error(String(lastErr?.message || lastErr));
  process.exit(2);
}
...
async function withBrowser(fn) {
  ...
  for (let i = 0; i < 2; i++) {
    try {
      ...
      if (!LIFECYCLE_ERR.test(msg) || i === 1) throw err;
      await wait(800);
    }
  }
  throw lastErr;
...
async function gotoWithRetry(page, url, attempts = 3) {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      lastErr = err;
      const msg = String(err.message || err);
      if (!LIFECYCLE_ERR.test(msg)) throw err;
      await wait(400 * (i + 1));
    }
  }
  throw lastErr;
...
async function reloadWithRetry(page, attempts = 2) {
  let lastErr;
  for (let i = 0; i < attempts; i++) {
    try {
...
async function newStablePage(browser, viewport) {
  let lastErr;
  for (let i = 0; i < 3; i++) {
    try {
...
const report = { pass: [], fail: [], spot: {}, pageErrors: [] };
...
try {
  await withBrowser(async (b) => {
    browser = b;
    let page = await newStablePage(browser, { width: 1440, height: 900 });
  ...
  await gotoWithRetry(page, `${base}/workbench.html`);
  ...
  const inv = await page.evaluate(() => ({
    ...
    stepLabels: [...document.querySelectorAll('.wb-field-label, label')].filter((el) =>
      /step\s*\d/i.test(el.textContent),
    ).length,
  ...
  if (
    inv.cardLabels.length === 5 &&
    inv.cardLabels[0]?.includes('005') &&
    inv.cardLabels.some((l) => l.includes('003')) &&
    inv.cardLabels.some((l) => l.includes('013')) &&
    inv.cardLabels.some((l) => l.includes('021'))
  )
    pass('case cards show full public archive set');
  else fail(`card labels: ${inv.cardLabels.join('|')}`);
  if (inv.activeProvCase?.includes('GAP 2.5/3') && /verified/i.test(inv.activeProvCase || ''))
    pass('active module full provenance');
  ...
  if (!report.spot.curatedPaste.gateVisible && report.spot.curatedPaste.pasteVisible)
    pass('curated paste opens without email gate');
  ...
  if (byoVal.yesSir === 'blocked' && byoVal.rule === 'allowed' && byoVal.safe === 'allowed')
    pass('BYO validateSurfacedAnswer logic');
  ...
  const suggestUi = await page.evaluate(() => ({
    ...
    suggestTab: [...document.querySelectorAll('button.wb-mode-btn')].some((b) =>
      b.textContent.includes('Suggest an Investigation'),
    ),
    ...
    openSuggestBtn: [...document.querySelectorAll('.wb-suggest-module button')].some((b) =>
      b.textContent.includes('Suggest an investigation'),
    ),
    ...
    fullSuggestNotBetweenFindingAndConfirm: (() => {
      ...
      const suggestBetween =
        !!(readout.compareDocumentPosition(suggest) & Node.DOCUMENT_POSITION_FOLLOWING) &&
  ...
  if (suggestUi.confirmFollowsFinding && suggestUi.suggestAfterConfirm && suggestUi.fullSuggestNotBetweenFindingAndConfirm)
    pass('finding → confirm → suggest order');
  else fail('suggest module placement wrong');
  if (suggestUi.suggestCollapsed && suggestUi.openSuggestBtn && suggestUi.suggestLead?.includes('Help expand the archive'))
    pass('readable collapsed suggest CTA');
  ...
  const suggestHeading = await page.evaluate(() => {
    ...
    const cta = [...document.querySelectorAll('.wb-suggest-module button')].find((b) =>
      b.textContent.includes('Suggest an investigation'),
    );
  ...
  if (suggestHeading.hasHeading && suggestHeading.headingText === 'Suggest an Investigation')
    pass('suggest section title present');
  else fail(`suggest section title: ${JSON.stringify(suggestHeading)}`);
  if (suggestHeading.headingFamily?.includes('Fraunces') && parseFloat(suggestHeading.headingSize) >= 22)
    pass('suggest title serif display style');
  else fail(`suggest title style: ${JSON.stringify(suggestHeading)}`);
  if (
    suggestHeading.eyebrowText === 'Field contribution' &&
    suggestHeading.headingColor === 'rgb(242, 232, 220)' &&
    suggestHeading.leadColor !== suggestHeading.headingColor &&
    suggestHeading.ctaPrimary
  )
    pass('title/body/CTA color hierarchy');
  ...
  await page.evaluate(() =>
    [...document.querySelectorAll('.wb-suggest-module button')]
      .find((b) => b.textContent.includes('Suggest an investigation'))
      ?.click(),
  );
  await wait(200);
  ...
  const formCheck = await page.evaluate(() => ({
    ...
    submitBtn: [...document.querySelectorAll('.wb-suggest-module button')]
      .find((b) => b.textContent.includes('Submit Investigation'))
      ?.textContent?.trim(),
  ...
  await page.evaluate(() => {
    ...
    window.fetch = async (url, opts) =>
      String(url).includes('/api/repository') ? { ok: true, json: async () => ({ ok: true }) } : window.__origFetch(url, opts);
  });
  ...
  if (!report.spot.suggestComplete.gauge && !report.spot.suggestComplete.score && !report.spot.suggestComplete.resultPage)
    pass('no score/result from suggest submit');
  ...
  await page.evaluate(() => {
    ...
    window.fetch = async (url) =>
      String(url).includes('/api/repository') ? { ok: false, json: async () => ({ ok: false }) } : window.__origFetch(url);
  });
  await reloadWithRetry(page);
  await wait(300);
  await page.evaluate(() =>
  ...
  if (report.pageErrors?.length) {
    fail(`page console errors: ${report.pageErrors.join(' | ')}`);
  }
  ...
  const msg = String(err.message || err);
  if (LIFECYCLE_ERR.test(msg)) {
    console.error('[final-check] Harness failure (Puppeteer lifecycle):', msg);
    process.exit(3);
  }
  console.error('[final-check] Unexpected error:', msg);
  throw err;
}
